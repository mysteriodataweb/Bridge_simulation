...
st.title("🏗️ Advanced Mechanical Analysis (MMC): Marine Bridge")
...
st.write("### 📊 Simulation Technical Data")
m1, m2, m3, m4 = st.columns(4)
m1.metric("Net Weight (MN)", f"{Weight_effective/1e6:.2f}", help="Total weight considering buoyancy")
m2.metric("Wind Force (kN)", f"{F_wind/1e3:.1f}")
m3.metric("Seismic Inertia (MN)", f"{F_seismic/1e6:.2f}")
m4.metric("Max Stress (MPa)", f"{sigma_max/1e6:.2f}", 
          delta=f"{(sigma_max/yield_limit)*100:.1f}% of limit", delta_color="inverse")

# --- 4. 3D VISUALIZATION ---
st.write("### 🧊 3D Model and Load Vectors")
fig = go.Figure()
...
def draw_pier(x_offset, color):
    ...
    fig.add_trace(go.Scatter3d(x=x_coords, y=np.zeros_like(z), z=z, mode='lines',
                               line=dict(color=color, width=D_piers*5), name="Pier"))
...
fig.add_trace(go.Mesh3d(x=[-L_deck, L_deck, L_deck, -L_deck], y=[-L_deck/2, -L_deck/2, L_deck/2, L_deck/2],
                        z=[H_wave, H_wave, H_wave, H_wave], color='cyan', opacity=0.2, name="Water Level"))
...
add_force(deflection_curve[10], 0, H_wave/2, scale_f*0.7, "Wave Force", "teal")
add_force(d_top/2, 0, H_piers/2, scale_f*0.5, "Seismic Inertia", "green")
...
if sigma_max > yield_limit:
    st.error(f"❌ COLLAPSE: Von Mises Stress ({sigma_max/1e6:.2f} MPa) exceeds Yield Strength (25 MPa)!")
else:
    st.success(f"✅ STABLE: Stress ({sigma_max/1e6:.2f} MPa) is within safety limits.")